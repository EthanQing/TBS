...
GPU 绑定由容器运行时和训练参数共同决定。`device=auto` 会继承容器内可见 CUDA 设备；显式 `device=0` / `device=1` 表示宿主 GPU 编号。Docker worker 如果设置了数字形式 `NVIDIA_VISIBLE_DEVICES`，训练入口会把宿主 GPU 编号映射到容器内本地编号，例如 `NVIDIA_VISIBLE_DEVICES=1` 且任务 `device=1` 时，训练子进程使用容器内 `CUDA_VISIBLE_DEVICES=0` 并传给框架本地设备 0。队列 worker 也会按该可见列表跳过不匹配的显式 GPU 任务。排查多 GPU 部署时，应在每个 worker 容器内检查 `NVIDIA_VISIBLE_DEVICES`、`CUDA_VISIBLE_DEVICES`、`nvidia-smi -L` 和 `torch.cuda.device_count()`，确认容器确实只看到预期 GPU。
