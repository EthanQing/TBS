...
- Worker：`WORKER_POLL_INTERVAL`、`WORKER_HEARTBEAT_INTERVAL`、`WORKER_STALE_AFTER_SECONDS`、`WORKER_BIND_HOST`、`WORKER_ID`、`NVIDIA_VISIBLE_DEVICES`
...
在外层 compose 部署中，YOLO 训练 worker 默认通过 `NVIDIA_VISIBLE_DEVICES` 和 `deploy.resources.reservations.devices.device_ids` 分别绑定宿主 GPU 0 / GPU 1。显式训练设备参数仍使用宿主 GPU 编号；worker 会在容器内映射为本地 CUDA 编号。
