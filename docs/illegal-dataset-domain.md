...
Dependency direction is API to the Illegal Dataset domain, then dataset storage policy, then platform filesystem primitives. The Illegal Dataset domain does not import `services/v3`; only the publishing workflow may depend on the legacy converter and `StandardDatasetService` during this migration phase.
