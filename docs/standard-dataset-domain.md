...
- `queries.py` owns statistics, preview and view payloads, file and annotation reads, and `.dataset_stats.json` / `.dataset_view_index.json`. These files are derived caches; dataset content and database rows remain authoritative.
- `events.py` owns Standard Dataset event creation and listing.

Illegal publishing and dataset augmentation depend directly on `standard.content.materialize_from_source_tree`. Upload orchestration calls copied or mounted Standard content capabilities without a legacy Standard service façade.
